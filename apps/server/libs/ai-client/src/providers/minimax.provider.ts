...
@Injectable()
export class MinimaxProvider implements IAIProviderAdapter {
  ...
  private getClient(): OpenAI {
    if (!this.client) {
      const config = this.configService.getAIProviderConfig('minimax');
      const apiKey = config.apiKey;

      if (!apiKey) {
        this.logger.warn(
          'MINIMAX_API_KEY is not configured in database or environment',
        );
      }
  ...
  private async blockingChat(
    ...
    try {
      ...
      this.logger.error(
        `MiniMax completion error: ${errorMessage}`,
        errorStack,
      );
      throw error;
    }
  ...
  private async *streamChat(
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(`MiniMax stream error: ${errorMessage}`, errorStack);
      throw error;
    }
  }
}
