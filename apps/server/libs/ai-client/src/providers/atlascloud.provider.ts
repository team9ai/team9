...
@Injectable()
export class AtlasCloudProvider implements IAIProviderAdapter {
  private readonly logger = new Logger(AtlasCloudProvider.name);
  ...
  private getClient(): OpenAI {
    if (!this.client) {
      const config = this.configService.getAIProviderConfig('atlascloud');
      const apiKey = config.apiKey;

      if (!apiKey) {
        this.logger.warn(
          'ATLASCLOUD_API_KEY is not configured in database or environment',
        );
      }
  ...
  private async blockingChat(
    ...
    try {
      ...
      return {
        provider: AIProvider.ATLASCLOUD,
        model: request.model,
      ...
      this.logger.error(
        `Atlas Cloud completion error: ${errorMessage}`,
        errorStack,
      );
      throw error;
    }
  ...
  private async *streamChat(
    ...
    try {
      ...
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(
        `Atlas Cloud stream error: ${errorMessage}`,
        errorStack,
      );
      throw error;
    }
