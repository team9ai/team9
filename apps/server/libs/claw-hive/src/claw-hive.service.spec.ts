...
describe('ClawHiveService', () => {
  ...
  describe('getAgent', () => {
    ...
    it('sends GET to /api/agents/:id with tenant header', async () => {
      mockFetch.mockResolvedValueOnce(jsonResponse(agentPayload));

      await service.getAgent('agent-abc', 'tenant-123');

      expect(mockFetch).toHaveBeenCalledWith(
        'http://test-hive:9999/api/agents/agent-abc',
        expect.objectContaining({
          headers: expect.objectContaining({
            'X-Hive-Auth': 'test-token',
    ...
    it('URL-encodes agent IDs with special characters', async () => {
      ...
      await service.getAgent('routine-creation/agent-123', 'tenant-123');

      const calledUrl = (mockFetch.mock.calls[0] as any[])[0] as string;
      expect(calledUrl).toBe(
        'http://test-hive:9999/api/agents/routine-creation%2Fagent-123',
      );
    });

    it('returns parsed agent data on success', async () => {
      mockFetch.mockResolvedValueOnce(jsonResponse(agentPayload));

      const result = await service.getAgent('agent-abc', 'tenant-123');

      expect(result).toEqual(agentPayload);
    });

    it('returns null when agent is not found (404)', async () => {
      mockFetch.mockResolvedValueOnce(textResponse('Not Found', 404));

      const result = await service.getAgent('missing-agent', 'tenant-123');

      expect(result).toBeNull();
    });

    it('throws when response is non-ok and not 404', async () => {
      mockFetch.mockResolvedValueOnce(textResponse('Internal Server Error', 500));

      await expect(service.getAgent('agent-abc', 'tenant-123')).rejects.toThrow(
        'getAgent failed: 500 Internal Server Error',
      );
    });
    ...
    it('omits X-Hive-Tenant header when tenantId is not provided', async () => {
      ...
      await service.getAgent('agent-abc');
