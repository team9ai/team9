...
export class ViewsService {
  ...
  /**
   * Fetch a paginated page of non-deleted root message IDs from the channel,
   * sorted by createdAt descending. Cursor is a message ID (UUID v7 — lexicographic
   * ordering matches time ordering for UUIDs generated by uuidv7).
   *
  ...
  private async findMessageIdsForView(params: {
    ...
    if (params.cursor) {
      // UUID v7 is lexicographically ordered by time; lt works as a time cursor
      conditions.push(lt(schema.messages.id, params.cursor));
    }
