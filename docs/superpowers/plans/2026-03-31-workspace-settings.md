...
it("updates workspace and invalidates workspace queries", async () => {
  ...
  await act(async () => {
    await result.current.mutateAsync({
      workspaceId: "ws-1",
      data: { name: "Renamed", slug: "renamed", logoUrl: "https://cdn/logo.png" },
    });
  });
  ...
  expect(invalidateQueries).toHaveBeenCalledWith({ queryKey: ["user-workspaces"] });
  expect(invalidateQueries).toHaveBeenCalledWith({ queryKey: ["workspace", "ws-1"] });
});
```
...
it("uploads a logo and saves the edited workspace", async () => {
  ...
  fireEvent.change(screen.getByLabelText(/^name$/i), { target: { value: "Renamed Workspace" } });
...
it("shows workspace settings only for owner or admin", () => {
  mockUseCurrentWorkspaceRole.mockReturnValue({ isOwner: false, isAdmin: false, isOwnerOrAdmin: false });
...
it("navigates to workspace settings from the workspace group", async () => {
  mockUseCurrentWorkspaceRole.mockReturnValue({ isOwner: false, isAdmin: true, isOwnerOrAdmin: true });
  render(<MoreMainContent />);
