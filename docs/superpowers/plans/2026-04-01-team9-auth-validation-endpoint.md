...
describe('InternalAuthController (integration)', () => {
  ...
  afterEach(async () => {
    if (originalSecret === undefined) delete process.env.INTERNAL_AUTH_VALIDATION_TOKEN;
...
describe('TokenValidatorService', () => {
  ...
  it('accepts the Team9 valid/botId/userId/tenantId response', async () => {
    await createModule({
      AUTH_VALIDATION_URL: 'http://team9.test/api/v1/internal/auth/validate-bot-token',
